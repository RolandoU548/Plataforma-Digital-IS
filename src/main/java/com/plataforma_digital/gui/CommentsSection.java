...
public class CommentsSection extends JPanel {
    ...
    public void updateComments() {
        ...
        List<Comment> comments = DatabaseConnection.getInstance().getAllCommentsByPublicationId(publication.getId());
        for (Comment comment : comments) {
            JPanel commentPanel = new JPanel();
            commentPanel.add(
                    new JLabel("Comment by: "
                            + DatabaseConnection.getInstance().getUserById(comment.getUserId()).getEmail()));
